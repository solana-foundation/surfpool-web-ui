...
description: Complete public API for the @solana/surfpool package — Surfnet class, config types, deploy options, and event shape.
---

The `@solana/surfpool` package exposes one class, `Surfnet`, plus the supporting types you need to call its methods type-safely. This page lists every exported symbol; for narrative usage, see the guides starting with [Overview](/sdk/overview).
...
interface SetTokenAccountUpdate {
  ...
  confidential?: ConfidentialTransferAccountUpdate; // Token-2022 only
}

// Configures the Token-2022 confidential-transfer extension on the account.
// Keys are the owner's, derived from their wallet signature.
interface ConfidentialTransferAccountUpdate {
  elgamalPubkey: string;         // required; base58/base64, 32 bytes
  aesKey: string;                // required; base58/base64, 16 bytes
  amount?: number;               // confidential available balance (base units); default 0
  approved?: boolean;            // default true
  allowConfidentialCredits?: boolean;    // default true
  allowNonConfidentialCredits?: boolean; // default true
  maximumPendingBalanceCreditCounter?: number; // default 65536
}
```
...
A flat object with a `kind` discriminator and optional fields. See [Events](/sdk/events) for the variant catalog.
...
If none of the platform packages installs on your machine, `Surfnet.start()` throws at first call with a clear "native module not found" error. See [Installation](/sdk/installation) for the support matrix and troubleshooting.
