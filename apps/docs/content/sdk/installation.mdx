---
title: Installation
description: Install the Surfpool SDK for Rust or TypeScript. The JS package ships pre-built native binaries through napi-rs for macOS and Linux.
---
...
The crate re-exports the Solana types you need most often, so a single `use surfpool_sdk::{...}` is usually enough:
...
- **Node.js 18 or newer.** The SDK uses `Uint8Array`, native `fetch`, and `assert/strict` from `node:test`.
- **A writable temp directory.** Each Surfnet instance allocates ports and may write transient data to the OS temp directory.
...
Run with `node --test smoke.test.ts` (with a TS loader) or `tsx --test smoke.test.ts`.
